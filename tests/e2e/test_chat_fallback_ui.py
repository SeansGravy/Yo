...
def test_ui_renders_fallback_reply(tmp_path: Path) -> None:
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            except Error as exc:  # pragma: no cover - depends on local setup
            ...
            try:
                ...
                page.fill("#message-input", "force fallback please")
                page.click("#send-button")
                page.wait_for_selector(".message.assistant .assistant-text", timeout=8000)
                assistant_text = page.text_content(".message.assistant:last-child .assistant-text") or ""
                status_text = page.text_content("#status") or ""
                assert assistant_text.strip() != "", "Assistant bubble was empty after fallback"
                assert "fallback reply received" in status_text.lower()
            finally:
        ...
        response = requests.post(
            ...
            json={
                "namespace": "default",
                "message": "force fallback please",
                "session_id": "ui-fallback-probe",
                "stream": True,
                "force_fallback": True,
            },
            timeout=5,
        ...
        assert reply_text, "HTTP fallback response did not include text"
